...
for space in spacing:
    ...
    ds_256 = yt.load(f'./results_mid_b_dens_grad_high/data_files_256/LinWave.out2.{space:05d}.athdf', units_override=unit_base)
    ...
    print(np.mean(magnetic_field_y))
    plt.pcolormesh(X, Y, magnetic_field_y, cmap='gray', shading='auto', vmin=12, vmax=16)
    ...
    plt.title(f'Magnetic Field Y Map (time={time_evolved:.4f})')
    plt.colorbar(label=r'Magnetic Field (\nu G)')
    plt.xlabel('x (pc)')
    plt.ylabel('y (pc)')
    plt.savefig(f'magnetic__field_y_map_{time_evolved:.4f}.png', dpi=300, bbox_inches='tight')
    plt.close()
