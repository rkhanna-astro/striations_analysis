...
velocity_plot_512 = np.zeros((int(size / inc), size))
# velocity_plot_2 = np.zeros((int(size / inc), size))
# velocity_plot_3 = np.zeros((int(size / inc), size))

averaged_number_density_512 = np.zeros((int(size / inc), size))
# averaged_number_density_1 = np.zeros((int(size / inc), size))
# averaged_number_density_2 = np.zeros((int(size / inc), size))


magnetic_plot_512 = np.zeros((int(size / inc), size))
# magnetic_plot_1 = np.zeros((int(size / inc), size))
# magnetic_plot_2 = np.zeros((int(size / inc), size))
...
for space in spacing:
    ...
    ds_512 = yt.load(f'./results_{name}/data_files_512/LinWave.out2.{space:05d}.athdf', units_override=unit_base)
    ...
    # right_edge = ds_512.domain_right_edge
    dims =  ds_512.domain_dimensions

    data_512 = ds_512.covering_grid(level=0, left_edge=left_edge, dims=dims)
    density_512 = data_512['rho'].to("g/cm**3")[:, :, 0].T

    accurate_number_density_512 = data_512['rho'].to("g/cm**3") / (2.34 * mh)
    number_density_512 = accurate_number_density_512.to("cm**-3")[:, :, 0].T

    velocity_x = data_512[('gas', 'velocity_x')].to("km/s")[:, :, 0].T
    velocity_y = data_512[('gas', 'velocity_y')].to("km/s")[:, :, 0].T

    net_velocity = np.sqrt(velocity_x**2 + velocity_y**2)

    time_evolved = ds_512.current_time.to("Myr")

    # vrms = np.sqrt(velocity_x**2 + velocity_y**2)

    magnetic_field_y = data_512[('gas', 'magnetic_field_y')].to("uG")[:, : , 0].T

    plot_velocity_x = np.zeros((int(size / inc), size))
    plot_velocity_y = np.zeros((int(size / inc), size))

    number_density = number_density_512 - np.mean(number_density_512)
    net_velocity = net_velocity - np.mean(net_velocity)
    magnetic_field_y = magnetic_field_y - np.mean(magnetic_field_y)
    ...
    for x in range(inc, size, inc):
        ...
        if space == spacing[0]:
            velocity_plot_512[int(x / inc)] = averaged_value
            averaged_number_density_512[int(x / inc)] = averaged_dens_value
            magnetic_plot_512[int(x / inc)] = averaged_mag_value
        # elif space == spacing[1]:
        #     velocity_plot_2[int(x / inc)] = averaged_value
        #     averaged_number_density_1[int(x / inc)] = averaged_dens_value
        #     magnetic_plot_1[int(x / inc)] = averaged_mag_value
...
for ind in range(inc, size, inc):
    ...
    den_signal_512 = averaged_number_density_512[i]
    # den_signal_1 = averaged_number_density_1[i]
    # den_signal_2 = averaged_number_density_2[i]

    vel_signal = velocity_plot[i]
    vel_signal_512 = velocity_plot_512[i]
    # vel_signal_1 = velocity_plot_2[i]
    # vel_signal_2 = velocity_plot_3[i]

    mag_signal = magnetic_plot[i]
    mag_signal = magnetic_plot_512[i]
    # mag_signal_1 = magnetic_plot_1[i]
    # mag_signal_2 = magnetic_plot_2[i]

    f1, D1 = welch(den_signal, fs=256, window= 'hann', scaling='spectrum', nperseg = 256, noverlap = 128)
    f2, D2 = welch(den_signal_512, fs=512, window= 'hann', scaling='spectrum', nperseg = 512, noverlap = 128)
    ...
    f1, V1 = welch(vel_signal, fs=256, window= 'hann', scaling='spectrum', nperseg = 256, noverlap = 128)
    f2, V2 = welch(vel_signal_512, fs=512, window= 'hann', scaling='spectrum', nperseg = 512, noverlap = 128)
    # f2, P2 = welch(den_signal_1, fs=256, window= 'hann', scaling='spectrum', nperseg = 256, noverlap = 128)
    # f3, P3 = welch(den_signal_2, fs=256, window= 'hann', scaling='spectrum', nperseg = 256, noverlap = 128)
    ...
    ax1.loglog(f1[:50], D1[:50], color='tab:grey', linewidth=0.7, label=r'256 $\times$ 256 density spectra')
    ax2.loglog(f1[:50], V1[:50], color='tab:blue', linewidth=0.7, label=r'256 $\times$ 256 velocity spectra')

    ax1.loglog(f2[:50], D2[:50], color='tab:red', linewidth=0.7, label=r'512 $\times$ 512 density spectra')
    ax2.loglog(f2[:50], V2[:50], color='tab:green', linewidth=0.7, label=r'512 $\times$ 512 velocity spectra')
    # ax1.loglog(f3[:25], P3[:25], color='tab:red', linewidth=0.7, label=f'{time[2]:.2f}')
    ax1.set_xlabel(r'spatial frequency $\bar{\nu} \,\, [pc^{-1}]$')
    ax1.set_ylabel(r'$|\mathrm{P_{n_{dens}}}|^{2}$', color='black')
    ax2.set_ylabel(r'$|\mathrm{P_{v}}|^{2}$', color='black')
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f'convergence_test_{(round(ind/256, 3)):.3f}_pc.pdf', format='pdf', bbox_inches='tight')
    plt.close()
