...
for space in spacing:
    ...
    B_pos_st_old = (np.sqrt(2 * np.pi * mean_density)* (rms_speed / np.sqrt(sigma_phi_old))).to("uG")
    B_pos_st_new = (np.sqrt(2 * np.pi * mean_density)* (rms_speed / np.sqrt(sigma_phi_new))).to("uG")
    B_pos_st_den = (np.sqrt(2 * np.pi * mean_density)* (rms_speed / np.sqrt(sigma_density))).to("uG")

    st_density.append(B_pos_st_den)
    st_expected.append(B_pos_st_new)
    st_original.append(B_pos_st_old)
...
plt.plot(time_in_myr, st_original, color=colors[1], lw=0.8, ls=linestyles[1], label=r'$B_{ST}, \delta \theta = \frac{\delta B_x}{B_0}$')
plt.plot(time_in_myr, st_expected, color=colors[2], lw=0.8, ls=linestyles[2], label=r'$B_{ST}, \delta \theta = \frac{\delta B_y}{B_0}$')
plt.plot(time_in_myr, st_density, color=colors[3], lw=0.8, ls=linestyles[3], label=r'$B_{ST}, \delta \theta = \frac{\delta \rho}{\rho_0}$')
...
plt.savefig(f"ST_method_magnetic_field_comparison_{name}.pdf", bbox_inches="tight")
