...
spacing = np.linspace(10, 500, 490, dtype=int)
# spacing = [0,36]
...
for space in spacing:
    ...
    # dispersion_angle =
    # print(physical_magnetic_field_y - np.mean(physical_magnetic_field_y))
    # angle = (np.abs(physical_magnetic_field_y - np.mean(physical_magnetic_field_y))) / np.mean(b_total)
    # # mean_dispersion = gaussian_filter(dispersion, smoothening)
    # # print(dispersion)
    # # net_dispersion = dispersion - mean_dispersion
    # net_dispersion = angle - np.mean(angle)
    
    # sigma_phi = np.sqrt(np.average(net_dispersion**2, weights= density))

    mean_density = gaussian_filter(number_density[int(size/4)], smoothening)
    mean_density = yt.YTArray(mean_density, 'cm**-3')
    density_fluctuations = np.abs(number_density[int(size/4)] - mean_density)

    density_contrast = density_fluctuations / mean_density
    print(density_contrast)
    sigma_phi = np.std(density_contrast)

    # print("Dispersion", sigma_contrast)
    ...
    mean_speed = np.average(np.sqrt(mean_velocity_x**2))

    print("MEAN velocity", mean_speed)
    ...
    B_pos_dcf = (Q * np.sqrt(4 * np.pi * mean_density) * (mean_speed / sigma_phi)).to("uG")
    B_pos_st = (np.sqrt(2 * np.pi * mean_density)* (mean_speed / np.sqrt(sigma_phi))).to("uG")

    dcf.append(B_pos_dcf)
    st.append(B_pos_st)
...
plt.plot(time_in_myr, dcf, color=colors[1], lw=0.8, ls=linestyles[1], label=r'$B_{DCF}$')
plt.plot(time_in_myr, st, color=colors[2], lw=0.8, ls=linestyles[2], label=r'$B_{ST}$')
...
plt.savefig(f"magnetic_field_comparison_{name}.pdf", bbox_inches="tight")
