...
for space in spacing:
    ...
    net_magnetic_field_y = magnetic_field_y - np.mean(magnetic_field_y)
    ...
    for x in range(16, size, 16):
            if abs((x / size) - 0.25) < tolerance:
                  case_1 = ((abs(net_magnetic_field_y[x]) / abs(magnetic_field_x[x])) + (abs(net_magnetic_field_y[x-1]) / abs(magnetic_field_x[x-1])) 
                            + (abs(net_magnetic_field_y[x+1]) / abs(magnetic_field_x[x+1]))) / 3.0

            elif abs((x / size) - 0.50) < tolerance:
                  case_2 = ((abs(net_magnetic_field_y[x]) / abs(magnetic_field_x[x])) + (abs(net_magnetic_field_y[x-1]) / abs(magnetic_field_x[x-1])) 
                            + (abs(net_magnetic_field_y[x+1]) / abs(magnetic_field_x[x+1]))) / 3.0

            elif abs((x / size) - 0.75) < tolerance:
                  case_3 = ((abs(net_magnetic_field_y[x]) / abs(magnetic_field_x[x])) + (abs(net_magnetic_field_y[x-1]) / abs(magnetic_field_x[x-1])) 
                            + (abs(net_magnetic_field_y[x+1]) / abs(magnetic_field_x[x+1]))) / 3.0
...
plt.ylabel(r'$B_y^{\prime} / B_x$', fontsize=10)
plt.yscale('log')
...
plt.savefig(f"mag_components_{name}_time_{curr_time}.pdf", bbox_inches="tight")
